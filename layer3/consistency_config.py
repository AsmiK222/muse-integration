"""
TreBle Respire — Consistency Configuration
...
Import this constant everywhere it is needed — do not redefine inline.
"""
...
# ── Recording duration — per Arvind decision 3 ─────────────────────────────
...
def required_recording_duration(respiratory_rate_bpm: float = None) -> float:
    ...
    if respiratory_rate_bpm is None:
        return EXTENDED_RECORDING_S   # unknown rate — be conservative
    if respiratory_rate_bpm < BRADYPNOEA_THRESHOLD_BPM:
        return EXTENDED_RECORDING_S
    return MIN_RECORDING_S


# ── Threshold provenance — per Arvind decision 2 ───────────────────────────
...
WHEEZE_THRESHOLD        = 0.243
WHEEZE_THRESHOLD_STATUS = "provisional"   # "provisional" | "calibrated"
...
THRESHOLD_PROVENANCE = {
    ...
    "wheeze": {
        ...
        "source": "in-distribution screening threshold — no Taal-specific value exists",
...
if __name__ == "__main__":
    ...
    print(f"  Normal rate (≥12 bpm) : {MIN_RECORDING_S}s")
